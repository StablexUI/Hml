		...
		$ret = null;
		{
			$_g = (new _hx_array(array()));
			{
				...
				while($_g1 < $_g2->length) {
					$child = $_g2[$_g1];
					++$_g1;
					if((is_object($_t3 = $child->nodeType) && !($_t3 instanceof Enum) ? $_t3 === Xml::$Element : $_t3 == Xml::$Element) && Xml_0($this, $_g, $_g1, $_g2, $_t, $child, $name, $ret) === $name) {
						$_g->push($child);
					}
					unset($child,$_t3);
				}
...
Xml::$Document = 6;
function Xml_0(&$__hx__this, &$_g, &$_g1, &$_g2, &$_t, &$child, &$name, &$ret) {
	{
		if((is_object($_t4 = $child->nodeType) && !($_t4 instanceof Enum) ? $_t4 !== Xml::$Element : $_t4 != Xml::$Element)) {
			throw new HException("Bad node type, expected Element but found " . _hx_string_rec($child->nodeType, ""));
		}
